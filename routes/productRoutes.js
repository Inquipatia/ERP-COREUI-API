...
router.get('/stats', requireAuth, requirePermission('products.view'), async (_request, response, next) => {
  try {
    response.json(await statisticsService.getInventoryStats('products'))
  } catch (error) {
    next(error)
  }
})
