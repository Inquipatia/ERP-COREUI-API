...
router.get('/stats', requireAuth, requirePermission('materials.view'), async (_request, response, next) => {
  try {
    response.json(await statisticsService.getInventoryStats('materials'))
  } catch (error) {
    next(error)
  }
})
