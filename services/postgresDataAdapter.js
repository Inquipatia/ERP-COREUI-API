...
const serializeWorkOrderMaterial = (record = {}) => {
  ...
  return {
    ...
    observations: record.observations || '',
    category: payload.category || 'Materiales y graficas',
    itemType: payload.itemType || 'material',
    status: payload.status || '',
    availability: payload.availability || '',
    isRequired: Boolean(payload.isRequired ?? payload.required),
    isRecommended: Boolean(payload.isRecommended ?? payload.recommended),
    confirmedForLoading: Boolean(payload.confirmedForLoading),
    loadedInVehicle: Boolean(payload.loadedInVehicle),
...
const serializeWorkOrderChecklistItem = (record = {}) => {
  ...
  return {
    ...
    observations: record.observations || '',
    itemType: payload.itemType || 'checklist',
    status: payload.status || '',
    availability: payload.availability || '',
...
const serializeWorkOrderEvidence = (record = {}) => ({
  ...
  category: record.payload?.category || record.payload?.evidenceType || '',
  evidenceType: record.payload?.evidenceType || record.payload?.category || '',
  payload: record.payload || null,
  createdAt: toIso(record.createdAt),
  updatedAt: toIso(record.updatedAt),
})
...
const serializeWorkOrder = (record = {}) => {
  ...
  return {
    ...
    installationAddress: payload.installationAddress || payload.address || payload.direccionInstalacion || '',
    commune: payload.commune || payload.comuna || '',
    scheduleAccess: payload.scheduleAccess || payload.horarioAcceso || '',
    supportSurface: payload.supportSurface || payload.surface || payload.superficie || payload.soporte || '',
    scheduledDate: payload.scheduledDate || payload.fechaProgramada || '',
    assignedPersonnel: payload.assignedPersonnel || payload.personalAsignado || '',
    includesInstallation: Boolean(payload.includesInstallation ?? payload.incluyeInstalacion),
    includesRemoval: Boolean(payload.includesRemoval ?? payload.incluyeRetiro),
    locationType: payload.locationType || '',
    vehicleType: payload.vehicleType || payload.vehiculo || '',
    priority,
    ...
    preparationDecisions: payload.preparationDecisions || {},
    preparationStatus: payload.preparationStatus || {},
    payload,
    createdAt: toIso(record.createdAt),
    updatedAt: toIso(record.updatedAt),
  }
